...
description: >-
  រៀនពីរបៀបគ្រប់គ្រងកំហុស (Error Handling) នៅក្នុង React និងការប្រើប្រាស់ Error
  Boundaries ដើម្បីការពារកុំអោយវេបសាយទាំងមូលគាំង (White Screen of Death)
  ពេលមានបញ្ហា។
objectives:
  - យល់ពីកំហុសដែលកើតឡើងនៅក្នុង Component Tree
  - >-
    ចេះប្រើប្រាស់បណ្ណាល័យ `react-error-boundary`
    ដើម្បីផ្តល់បទពិសោធន៍ល្អដល់អ្នកប្រើប្រាស់
  - 'ដឹងពីអ្វីដែល Error Boundaries មិនអាចចាប់បាន (API Errors, Event Handlers)'
module: module-7-advanced
order: 4
quiz:
  - question: >-
      ប្រសិនបើអ្នកមានកំហុស (Error) នៅក្នុងការហៅ API (`fetch`)
      ឫនៅក្នុងការចុចប៊ូតុង (`onClick`) តើ Error Boundary អាចចាប់យកកំហុសនោះ
      និងបង្ហាញផ្ទាំងជំនួស (Fallback UI) ដែរឬទេ?
    options:
      - '"អាចចាប់បានគ្រប់កំហុសទាំងអស់នៅក្នុង React"'
      - >-
        "មិនអាចចាប់បានទេ! Error Boundary ចាប់បានតែកំហុសដែលកើតឡើងក្នុងពេលកំពុង
        'គូររូបរាង (Rendering)' ប៉ុណ្ណោះ។ សម្រាប់កំហុស API ឫការចុចប៊ូតុង
        អ្នកត្រូវប្រើ `try/catch` ដោយខ្លួនឯង។"
      - '"អាចចាប់បាន លុះត្រាតែអ្នកប្រើ Redux"'
      - '"អាចចាប់បាន បើដាក់វានៅក្រៅគេបង្អស់"'
    correctAnswer: 2
    explanation: >-
      នេះគឺជាការយល់ច្រឡំដ៏ពេញនិយមបំផុត! Error Boundaries
      ប្រៀបដូចជាការដាក់សំណាញ់ការពារអ្នកដើរសៀក (Rendering) អញ្ចឹង។
      ប៉ុន្តែបើអ្នកយកកាំភ្លើងបាញ់ខ្លួនឯង (ចុចប៊ូតុងមាន Error ឫហៅ API ខុស)
      សំណាញ់នោះមិនអាចជួយអ្នកបានទេ ត្រូវចេះសង្គ្រោះបឋមខ្លួនឯង (try/catch)!
---
