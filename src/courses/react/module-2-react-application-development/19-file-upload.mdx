---
title: File Upload
description: រៀនពីបច្ចេកទេសក្នុងការបញ្ជូនឯកសារ (រូបភាព ឫ ឯកសារផ្សេងៗ) ពីកុំព្យូទ័ររបស់អ្នកប្រើប្រាស់ ទៅកាន់ម៉ាស៊ីនមេ (Server) តាមរយៈការប្រើប្រាស់ FormData។
objectives:
...
order: 19
quiz:
  - question: "ហេតុអ្វីបានជាយើងត្រូវប្រើប្រាស់ `FormData` ជំនួសអោយ `JSON.stringify()` នៅពេលធ្វើការបញ្ជូនឯកសារ (File Upload) ទៅកាន់ API?"
    options:
      - "\"ព្រោះទិន្នន័យឯកសារ (រូបភាព) គឺជាកូដ Binary ដែលមិនអាចបម្លែងទៅជាអក្សរ JSON ធម្មតាបានទេ។ FormData អនុញ្ញាតអោយយើងបញ្ជូនវាជាទម្រង់ multipart/form-data បានដោយផ្ទាល់។\""
      - "\"ព្រោះ FormData មានល្បឿនលឿនជាង JSON\""
      - "\"ព្រោះ React មិនស្គាល់ JSON ទេ\""
      - "\"ដើម្បីការពារមេរោគ\""
    correctAnswer: 1
    explanation: "ឯកសារ (រូបភាព, វីដេអូ) មិនមែនជាអក្សរទេ ពួកវាជាប្លុកទិន្នន័យ (Binary)។ ការព្យាយាមបញ្ជូនវាជា JSON នឹងធ្វើអោយវាខូចឫគាំង។ `FormData` គឺជាស្តង់ដាររបស់ Web សម្រាប់ដោះស្រាយបញ្ហានេះ។"
---
